...
async def on_ready():
    ...
    await bot.command_channel.send(f"> `[{bot.machine_id}]` -📝 The OCR (Write) bot is **online**. ✨")
...
async def disconnect(ctx):
    await bot.command_channel.send(f"> `[{bot.machine_id}]` - 📝 The OCR bot (Write) is **shutting down**. 💢")
